...
Table.prototype.update = function (update, where, callback) {
    ...
    var name = this.name;
    QueryHelper.toKeyValue(update, function(err, updateKeys, updateValues){
        if(err){
            callback(err);
        }

        QueryHelper.toKeyValue(where, function(err, whereKeys, values){
            if(err){
                throw err;
            }

            for(var i = 0; i < updateValues.length; i++){
                values.push(updateValues[i]);
            }
